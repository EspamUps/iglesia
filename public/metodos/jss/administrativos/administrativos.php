...
function limpiarFormIngresarAdministrativo()
{
    $('#formIngresoResponsable').each(function () {
        this.reset();
    });
    setTimeout(function() {$("#mensajeFormIngresoResponsable").html('');},1500);
}
...
function EliminarCargoAsignado(idAdministrativo,ID){
    if (confirm('¿DESEAS ELIMINAR EL CARGO ASIGNADO?')) {
        var url = $("#rutaBase").text();
        $.ajax({
            url : url+'/administrativos2/eliminaradministrativo',
            type: 'post',
            dataType: 'JSON',
            data: { id: idAdministrativo},
            beforeSend: function () {
                $("#btnEliminarAdministrativo" + ID).html('<i class="fa fa-spinner"></i>');
                $("#mensajeTablaResponsables").html('');
            },
            uploadProgress: function (event, position, total, percentComplete) {
            },
